...
language_selector = st.sidebar.selectbox(
    ...
    ["English", "Français", "Español", "Deutsch", "العربية", "中文", "हिंदी", "Português"],
...
# Step 2: Date of Birth
    ...
    st.header(t("registration"))
    dob = st.text_input(t("date_of_birth"), key="date_of_birth")
    if st.button(t("next")) and dob:
        st.session_state.responses["date_of_birth"] = dob
...
# Step 3: Has Symptoms
    ...
    if st.button(t("next")) and has_symptoms:
        ...
        if has_symptoms == t("yes"):
            st.session_state.current_step = 4  # Go to symptoms description
        else:
            st.session_state.current_step = 7  # Skip to medical history
        st.rerun()

# Step 4: Symptoms Description (only if has symptoms)
...
# Step 7: Last Period
    ...
    st.header(t("registration"))
    days_since_period = st.number_input(
        t("last_period"), 
        min_value=0, 
        max_value=365, 
        value=0,
    ...
    if st.button(t("next")):
        st.session_state.responses["last_period"] = str(days_since_period) + " days ago"
...
# Step 8: Medical History
    ...
    if st.button(t("next")) and medical_history:
        ...
        if st.session_state.responses.get("has_symptoms") == t("no"):
            st.session_state.current_step = 12  # Skip to summary confirmation
        else:
            st.session_state.current_step = 9  # Continue with smoking
        st.rerun()

# Step 9: Smoking Status (only if has symptoms)
...
# Step 10: Recreational Drugs (only if has symptoms)
...
# Step 11: Alcohol Consumption (only if has symptoms)
...
# Step 12: Summary Confirmation
    ...
    if st.session_state.responses.get("has_symptoms") == t("yes"):
        summary_items.extend([
            ...
            f"{t('last_period_label')}: {st.session_state.responses.get('last_period')}",
...
# Step 13: Triage Classification via LLM
    ...
    # Modified: Remove consultation mode recommendation from prompt
    prompt_priority = (
...
# Step 14: Additional Booking Details
    ...
    if needs_translator == t("yes"):
        translator_language = st.text_input(t("translator_language"), key="translator_language_input")
...
# Step 15: Appointment Slot Selection & Booking
